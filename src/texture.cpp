...
namespace CGL {
  ...
  Color Texture::sample(const SampleParams& sp) {
    ...
    if (sp.lsm == CGL::L_ZERO) {
      level = 0;
      if (sp.psm == CGL::P_NEAREST)
    ...
    else if (sp.lsm == CGL::L_LINEAR) {
      if (sp.psm == CGL::P_NEAREST) {
        Color c1 = sample_nearest(sp.p_uv, ceil(level));
        if (ceil(level) == floor(level))
          return c1;
        
        Color c2 = sample_nearest(sp.p_uv, floor(level));
